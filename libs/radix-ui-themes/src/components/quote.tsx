...
const Quote = React.forwardRef<QuoteElement, QuoteProps>((props, forwardedRef) => {
  const { asChild, children: _children, className, ...quoteProps } = extractProps(
    props,
    quotePropDefs,
  );
  const Comp = asChild ? Slot.Root : 'q';
  return (
    <Comp {...quoteProps} ref={React.coerceRef(forwardedRef)} class={classNames('rt-Quote', className)}>
