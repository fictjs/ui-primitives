            ...
            <Box flexGrow="1" p="5" position="relative">
              <Box position="absolute" top="0" right="0" m="2">
                <Tooltip
                  ...
                  sideOffset={6}
                >
                  <Kbd asChild size="3" tabIndex={0}>
                    <button
                      type="button"
                      class="rt-ThemePanelShortcut"
...
function ThemePanelContent(props: ThemePanelContentProps): React.ReactNode {
  ...
  const copyState = createSignal<'idle' | 'copying' | 'copied'>('idle')

  const autoMatchedGray = () => getMatchingGrayColor(themeContext.accentColor)
  const resolvedGrayColor = () =>
    themeContext.grayColor === 'auto' ? autoMatchedGray() : themeContext.grayColor

  const handleAppearanceChange = (value: 'light' | 'dark') => {
    const cleanup = disableAnimation()

    if (themeContext.appearance !== 'inherit') {
  ...
  const handleCopyThemeConfig = async () => {
    const theme = {
      appearance:
      ...
      grayColor:
        themeContext.grayColor === themePropDefs.grayColor.default ? undefined : themeContext.grayColor,
      panelBackground:
        themeContext.panelBackground === themePropDefs.panelBackground.default
          ? undefined
          : themeContext.panelBackground,
      radius:
        themeContext.radius === themePropDefs.radius.default ? undefined : themeContext.radius,
      scaling:
        themeContext.scaling === themePropDefs.scaling.default ? undefined : themeContext.scaling,
    }
    ...
    copyState('copying')
    await navigator.clipboard.writeText(themeProps ? `<Theme ${themeProps}>` : '<Theme>')
    copyState('copied')
  ...
  createEffect(() => {
    ...
    const update = () => {
      ...
      if (themeContext.appearance === 'inherit') {
        resolvedAppearance(hasDarkClass ? 'dark' : 'light')
      } else {
        resolvedAppearance(themeContext.appearance)
      }
    }
    ...
    <Box {...contentProps}>
          ...
          >
            <Tooltip
              content={`${upperFirst(color)}${
                themeContext.accentColor === 'gray' && resolvedGrayColor() !== 'gray'
            ...
            >
              <input
                ...
                name="accentColor"
                value={color}
                checked={themeContext.accentColor === color}
                onChange={(event) => {
                  const target = event.currentTarget as HTMLInputElement
                  themeContext.onAccentColorChange(target.value as typeof themeContext.accentColor)
                }}
              />
            </Tooltip>
          </label>
            ...
            >
              ...
              >
                <input
                  ...
                  name="grayColor"
                  value={gray}
                  checked={themeContext.grayColor === gray}
                  onChange={(event) => {
                    const target = event.currentTarget as HTMLInputElement
                    themeContext.onGrayColorChange(target.value as typeof themeContext.grayColor)
                  }}
                />
              </Tooltip>
            </label>
      ...
      <Grid columns="2" gap="2" mt="3" role="group" aria-labelledby="appearance-title">
        {(['light', 'dark'] as const).map((value) => (
          <label key={value} class="rt-ThemePanelRadioCard">
            <input
              ...
              onChange={(event) => {
                const target = event.currentTarget as HTMLInputElement
                handleAppearanceChange(target.value as 'light' | 'dark')
              }}
            />
            <Flex align="center" justify="center" height="32px" gap="2">
              {value === 'light' ? <LightIcon /> : <DarkIcon />}
      ...
      <Grid columns="5" gap="2" mt="3" role="group" aria-labelledby="radius-title">
        {themePropDefs.radius.values.map((value) => (
          <Flex key={value} direction="column" align="center">
            <label class="rt-ThemePanelRadioCard">
              <input
                class="rt-ThemePanelRadioCardInput"
                type="radio"
                name="radius"
                ...
                value={value}
                checked={themeContext.radius === value}
                onChange={(event) => {
                  const target = event.currentTarget as HTMLInputElement
                  themeContext.onRadiusChange(target.value as typeof themeContext.radius)
                }}
              />
        ...
        {themePropDefs.scaling.values.map((value) => (
          <label key={value} class="rt-ThemePanelRadioCard">
            <input
              ...
              name="scaling"
              value={value}
              checked={themeContext.scaling === value}
              onChange={(event) => {
                const target = event.currentTarget as HTMLInputElement
                themeContext.onScalingChange(target.value as typeof themeContext.scaling)
              }}
            />
      ...
      <Flex mt="5" align="center" gap="2">
        ...
        <Popover.Root>
          ...
          <Popover.Content size="1" style={{ maxWidth: 220 }} side="top" align="center">
            <Text as="p" size="2">
              Whether Card and Table panels are translucent, showing some of the background behind
              them.
            </Text>
          </Popover.Content>
        </Popover.Root>
        ...
        {themePropDefs.panelBackground.values.map((value) => (
          <label key={value} class="rt-ThemePanelRadioCard">
            <input
              ...
              value={value}
              checked={themeContext.panelBackground === value}
              onChange={(event) => {
                const target = event.currentTarget as HTMLInputElement
                themeContext.onPanelBackgroundChange(
                  target.value as typeof themeContext.panelBackground,
                )
              }}
            />
            <Flex align="center" justify="center" height="32px" gap="2">
              {value === 'solid' ? <SolidPanelIcon /> : <TranslucentPanelIcon />}
              <Text size="1" weight="medium">
                {upperFirst(value)}
              </Text>
            </Flex>
